  ...
  return <main className="min-h-screen bg-[#f7faf8] text-neutral-950">
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-14"><div className="grid gap-8 lg:grid-cols-1"><aside className="lg:flex lg:items-end lg:justify-between lg:gap-8"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-teal-950 sm:text-sm">Reserva online</p><h1 className="mt-3 max-w-xl font-display text-4xl font-bold leading-[1.05] text-neutral-950 sm:mt-4 sm:text-6xl">Agenda tu estilo</h1><p className="mt-4 max-w-2xl text-base leading-7 text-neutral-600 sm:mt-5 sm:text-lg sm:leading-8">Reserva tu próxima visita en {settings?.business_name ?? "ELYON BARBER"} con una experiencia simple, clara y pensada para mobile.</p></div></aside>
      <DarkPanel className="min-h-[560px] overflow-hidden"><div className="mb-6 flex gap-2 overflow-x-auto pb-2 no-scrollbar">{(["services", "datetime", "contact"] as const).map((key, index) => <div key={key} className={`min-w-fit rounded-full border px-3.5 py-2 text-[11px] font-black transition-all duration-200 sm:px-4 sm:text-xs ${step === key ? "border-teal-950 bg-teal-950 text-white shadow-sm" : "border-teal-100 bg-white text-neutral-500"}`}>{index + 1}. {key === "services" ? "Servicios" : key === "datetime" ? "Fecha y hora" : "Datos"}</div>)}</div>
        {step === "services" && <section key="services" className="animate-[step-in_220ms_ease-out]"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-display text-3xl font-bold sm:text-4xl">Servicios</h2><p className="mt-2 max-w-xl text-sm leading-6 text-neutral-500 sm:text-base">Elige el servicio que quieres agendar con ELYON BARBER.</p></div></div>{error && <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p>}<div className="mt-7 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{services.map((item) => <article key={item.id} className="group flex min-h-[390px] flex-col overflow-hidden rounded-[1.35rem] border border-neutral-200/90 bg-white p-5 shadow-[0_10px_30px_-22px_rgba(0,0,0,0.45)] transition duration-200 hover:-translate-y-1 hover:border-teal-100 hover:shadow-[0_18px_40px_-24px_rgba(0,68,65,0.28)] sm:p-6"><div className="flex min-h-0 flex-1 flex-col"><div className="flex items-start justify-between gap-3"><h3 className="min-w-0 flex-1 text-[1.2rem] font-black leading-[1.12] tracking-[-0.025em] text-neutral-950 sm:text-[1.3rem]">{item.name}</h3><span className="shrink-0 rounded-full bg-teal-50 px-2.5 py-1.5 text-[9px] font-black uppercase tracking-[0.08em] text-teal-950">{durationLabel(item.duration_minutes)}</span></div><div className="mt-6 border-t border-neutral-100 pt-5"><p className="text-[2rem] font-black leading-none tracking-[-0.04em] text-teal-950 sm:text-[2.15rem]">{formatPrice(item.price)}</p><p className="mt-4 min-h-[4.5rem] text-sm leading-6 text-neutral-500">{item.description}</p></div></div><Button className="mt-6 min-h-12 w-full rounded-xl" onClick={() => setPendingService(item)}>Agendar servicio</Button></article>)}</div></section>}
