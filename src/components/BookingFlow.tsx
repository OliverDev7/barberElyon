...
function Modal({ title, children, onClose, compact = false }: { title: string; children: ReactNode; onClose: () => void; compact?: boolean }) {
  return (
    <div className={`fixed inset-0 z-50 grid place-items-end bg-neutral-950/45 p-0 backdrop-blur-sm ${compact ? "sm:place-items-end sm:p-0" : "sm:place-items-center sm:p-6"}`} onClick={onClose}>
      <section className={`max-h-[92vh] w-full overflow-auto bg-white p-5 shadow-2xl animate-[sheet-in_220ms_ease-out] ${compact ? "rounded-t-3xl sm:rounded-t-3xl sm:rounded-b-none sm:p-5" : "rounded-t-3xl sm:max-w-lg sm:rounded-3xl sm:p-7"}`} onClick={(event) => event.stopPropagation()}>
        ...
        {compact && <div className="flex justify-end">
          <button aria-label="Cerrar" className="focus-ring grid h-9 w-9 place-items-center rounded-full bg-neutral-100 text-lg font-black text-neutral-600 transition hover:bg-teal-50 hover:text-teal-950" onClick={onClose}>×</button>
        </div>}
...
export function BookingFlow() {
  ...
  function validateForm() {
    ...
    if (!/^\d{11}$/.test(form.phone.trim())) next.phone = "Ingresa un teléfono chileno válido. Debe tener 11 números.";
  ...
  function handlePhoneChange(value: string) {
    ...
    setFieldErrors((current) => {
      const next = { ...current };
      if (hasInvalidCharacters) next.phone = "Este campo solo admite numeros";
  ...
  return <main className="min-h-screen bg-[#f7faf8] text-neutral-950">
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-14"><div className="grid gap-8 lg:grid-cols-1"><aside className="lg:flex lg:items-end lg:justify-between lg:gap-8"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-teal-950 sm:text-sm">Reserva online</p><h1 className="mt-3 max-w-xl font-display text-4xl font-bold leading-[1.05] text-neutral-950 sm:mt-4 sm:text-6xl">Agenda tu estilo</h1><p className="mt-4 max-w-2xl text-base leading-7 text-neutral-600 sm:mt-5 sm:text-lg sm:leading-8">Reserva tu próxima visita en {settings?.business_name ?? "ELYON BARBER"} con una experiencia simple, clara y pensada para mobile.</p></div></aside>
      <DarkPanel className="min-h-[560px] overflow-hidden"><div className="mb-6 flex gap-2 overflow-x-auto pb-2 no-scrollbar">{(["services", "datetime", "contact"] as const).map((key, index) => <div key={key} className={`min-w-fit rounded-full border px-3.5 py-2 text-[11px] font-black transition-all duration-200 sm:px-4 sm:text-xs ${step === key ? "border-teal-950 bg-teal-950 text-white shadow-sm" : "border-teal-100 bg-white text-neutral-500"}`}>{index + 1}. {key === "services" ? "Servicios" : key === "datetime" ? "Fecha y hora" : "Datos"}</div>)}
      </div>
        {step === "services" && <section key="services" className="animate-[step-in_220ms_ease-out]"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-display text-3xl font-bold sm:text-4xl">Servicios</h2><p className="mt-2 max-w-xl text-sm leading-6 text-neutral-500 sm:text-base">Elige el servicio que quieres agendar con ELYON BARBER.</p></div></div>{error && <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p>}<div className="mt-7 grid gap-5 sm:grid-cols-2 xl:grid-cols-2">{services.map((item) => <article key={item.id} className="group flex h-[212px] min-h-[212px] flex-col overflow-hidden rounded-xl border border-neutral-300 bg-white p-5 shadow-[0_6px_18px_-16px_rgba(0,0,0,0.45)] transition duration-200 hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-[0_12px_26px_-18px_rgba(0,68,65,0.28)] sm:p-6"><div className="flex min-h-0 flex-1 flex-col"><div className="flex items-start justify-between gap-4"><div className="min-w-0 flex-1"><h3 className="text-[1rem] font-semibold leading-6 text-neutral-900 sm:text-[1.05rem]">{item.name}</h3><p className="mt-1.5 text-sm font-medium leading-5 text-neutral-400">{durationLabel(item.duration_minutes)}</p></div></div><p className="mt-1 text-sm font-bold text-neutral-900">{formatPrice(item.price)}</p><p className="mt-5 min-h-0 line-clamp-2 text-xs leading-5 text-neutral-500 sm:text-[13px]">{item.description}</p></div><Button className="mt-auto min-h-9 self-end rounded-md px-3.5 py-2 text-xs sm:min-h-9 sm:px-3.5 sm:py-2" onClick={() => setPendingService(item)}>Agendar servicio</Button></article>)}</div></section>}
        {step === "datetime" && service && <section key="datetime" className="animate-[step-in_220ms_ease-out]"><div className="text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-teal-950 text-lg font-black text-white shadow-lg shadow-teal-950/10">B.A.</div><p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-teal-950 sm:text-sm">{settings?.barber_name ?? "Barbero Alonso Salinas"}</p><h2 className="mt-3 font-display text-2xl font-bold sm:text-3xl">Selecciona fecha y hora</h2></div>
          <div className="mt-7 rounded-2xl border border-neutral-100 bg-white p-3 shadow-sm sm:mt-8 sm:p-6"><div className="mb-5 flex items-center justify-between gap-3"><h3 className="min-w-0 text-lg font-black capitalize leading-tight text-neutral-800 sm:text-3xl">{monthLabel(visibleDates)}</h3><button type="button" onClick={goToToday} className="focus-ring shrink-0 rounded-full border border-teal-100 px-3 py-2 text-xs font-black text-teal-950 transition hover:bg-teal-50 sm:px-4">Hoy</button></div><div className="grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-1 sm:grid-cols-[48px_minmax(0,1fr)_48px] sm:gap-3"><button aria-label="Fechas anteriores" className="focus-ring grid h-11 w-11 place-items-center rounded-full bg-neutral-50 text-3xl font-light text-neutral-700 transition hover:bg-teal-50 disabled:cursor-not-allowed disabled:opacity-30 sm:h-12 sm:w-12" type="button" onClick={() => moveCalendar(-1)} disabled={isSameDate(viewStart, today)}>‹</button><div className="grid min-w-0 grid-cols-7 gap-1 sm:gap-2">{visibleDates.map((item) => { const selected = isSameDate(item, date); return <button type="button" key={dateKey(item)} onClick={() => chooseDate(item)} aria-pressed={selected} className={`focus-ring mx-auto flex h-[82px] w-full min-w-0 flex-col items-center justify-center rounded-2xl text-center transition duration-200 sm:h-28 sm:max-w-[92px] ${selected ? "bg-[#26343b] text-white shadow-lg shadow-slate-900/10 scale-[1.02]" : "bg-white text-neutral-900 hover:bg-teal-50"}`}><span className={`max-w-full px-0.5 text-[10px] font-bold uppercase sm:text-sm ${selected ? "text-white/80" : "text-neutral-500"}`}>{item.toLocaleDateString("es-CL", { weekday: "short" }).replace(".", "")}</span><span className="mt-1 text-[1.65rem] font-black leading-none sm:text-3xl">{item.getDate()}</span><span className={`mt-1 text-[10px] sm:text-xs ${selected ? "text-white/70" : "text-neutral-400"}`}>{item.toLocaleDateString("es-CL", { month: "short" }).replace(".", "")}</span></button>; })}</div><button aria-label="Fechas posteriores" className="focus-ring grid h-11 w-11 place-items-center rounded-full bg-neutral-50 text-3xl font-light text-neutral-700 transition hover:bg-teal-50 sm:h-12 sm:w-12" type="button" onClick={() => moveCalendar(1)}>›</button></div><p className="mt-4 text-center text-[11px] leading-5 text-neutral-400 sm:text-xs">Desplázate por las fechas para continuar hacia los próximos días y meses.</p></div><div className="mt-7 grid gap-6 sm:mt-8">{[["morning", "Mañana"],["afternoon", "Tarde"],["night", "Noche"]].map(([key, label]) => <div key={key}><h3 className="mb-3 text-xs font-black uppercase tracking-[0.18em] text-neutral-500 sm:text-sm">{label}</h3><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{grouped[key as keyof typeof grouped].map((slot) => <button type="button" key={slot.id} onClick={() => { setTime(slot.time_24); setFieldErrors((current) => { const next = { ...current }; delete next.time; return next; }); }} className={`focus-ring min-h-12 rounded-xl border px-3 text-sm font-black transition ${time === slot.time_24 ? "border-teal-950 bg-teal-950 text-white shadow-sm" : "border-neutral-200 bg-white hover:bg-teal-50"}`}>{formatTime(slot.time_24)}</button>)}</div></div>)}</div>{availabilityLoading && <p className="mt-5 rounded-xl border border-teal-100 bg-white px-4 py-3 text-sm font-bold text-neutral-500">Consultando disponibilidad…</p>}{!availabilityLoading && slots.length === 0 && <p className="mt-5 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">No hay horas disponibles para este día.</p>}<p className="mt-5 rounded-xl border border-teal-100 bg-teal-50 px-4 py-3 text-sm font-bold text-teal-950">Las horas disponibles podrían agotarse, ¡agenda lo antes posible!</p><div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button variant="ghost" onClick={() => setStep("services")}>Volver</Button><Button disabled={!time || availabilityLoading} onClick={() => setStep("contact")}>Continuar</Button></div></section>}
        {step === "contact" && service && <section key="contact" className="animate-[step-in_220ms_ease-out]"><div className="rounded-2xl border border-teal-100 bg-teal-50/70 p-5 sm:p-6"><h2 className="font-display text-2xl font-bold sm:text-3xl">Resumen</h2><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><p><b>Servicio:</b> {service.name}</p><p><b>Fecha:</b> {dateLabel}</p><p><b>Hora:</b> {formatTime(time)}</p><p><b>Barbero:</b> {settings?.barber_name}</p><p className="text-xl font-black text-teal-950 sm:col-span-2">Precio: {formatPrice(service.price)}</p></div></div><div className="mt-7 grid gap-4 sm:grid-cols-2">{[["name", "Nombre *"],["lastName", "Apellido *"],["email", "Correo *"],["phone", "Teléfono *"]].map(([key, label]) => <label key={key} className="grid gap-2 text-sm font-bold text-neutral-700">{label}<input className={`focus-ring min-h-12 rounded-xl border px-4 transition ${fieldErrors[key] ? "border-red-300 bg-red-50" : form[key as keyof typeof form] ? "border-teal-200 bg-white" : "border-neutral-200 bg-white"}`} value={form[key as keyof typeof form]} onBlur={validateForm} onChange={(event) => { if (key === "phone") handlePhoneChange(event.currentTarget.value); else { setForm({ ...form, [key]: event.target.value }); setFieldErrors((current) => { const next = { ...current }; delete next[key]; return next; }); } }} onPaste={key === "phone" ? handlePhonePaste : undefined} maxLength={key === "phone" ? 11 : undefined} inputMode={key === "phone" ? "numeric" : undefined} pattern={key === "phone" ? "[0-9]*" : undefined} type={key === "phone" ? "tel" : "text"} autoComplete={key === "email" ? "email" : key === "phone" ? "tel" : "off"} />{fieldErrors[key] && <span className="text-xs font-bold text-red-700">{fieldErrors[key]}</span>}</label>)}<label className="grid gap-2 text-sm font-bold text-neutral-700 sm:col-span-2">Observaciones <span className="font-normal text-neutral-500">(opcional)</span><textarea className="focus-ring min-h-32 resize-none rounded-xl border border-neutral-200 px-4 py-3" value={form.comment} onChange={(event) => setForm({ ...form, comment: event.target.value })} /></label></div>{error && <p className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-700">{error}</p>}<div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between"><Button variant="ghost" onClick={() => setStep("datetime")}>Volver</Button><Button disabled={loading || !canBook} onClick={book}>{loading ? "Agendando..." : "Agendar"}</Button></div></section>}
      </DarkPanel></div></section>
    <PublicFooter />
    {pendingService && <Modal title="" compact onClose={() => setPendingService(null)}><div className="mx-auto w-full max-w-[760px] rounded-2xl border border-teal-100 bg-white px-5 py-4 shadow-[0_12px_35px_-24px_rgba(0,68,65,0.4)] sm:px-6 sm:py-4"><div className="flex flex-col gap-4 pr-1 sm:flex-row sm:items-center sm:gap-7"><div className="min-w-0 flex-1"><h3 className="text-lg font-black leading-tight tracking-[-0.02em] text-neutral-950 sm:text-xl">¿Estás a punto de agendar {pendingService.name}?</h3><p className="mt-1.5 text-sm font-semibold text-neutral-500"><strong className="text-neutral-700">Duración:</strong> {durationLabel(pendingService.duration_minutes)}</p></div><Button className="w-full shrink-0 sm:w-auto sm:min-w-[132px]" onClick={confirmService}>Confirmar</Button></div></div></Modal>}
    ...
    {modal === "hours" && <Modal title="Horario" onClose={() => setModal(null)}><div className="grid gap-3"><div className="rounded-2xl border border-teal-100 bg-teal-50/70 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-teal-950">Lunes a sábado</p><p className="mt-1.5 text-lg font-black text-neutral-950">9:00 hrs a 20:00 hrs</p></div><div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-neutral-500">Domingo</p><p className="mt-1.5 text-lg font-black text-neutral-950">Cerrado</p></div></div></Modal>}
