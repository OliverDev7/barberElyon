...
function Modal({ title, children, onClose, compact = false }: { title: string; children: ReactNode; onClose: () => void; compact?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-end bg-neutral-950/45 p-0 backdrop-blur-sm sm:place-items-center sm:p-6" onClick={onClose}>
      <section className={`max-h-[92vh] w-full overflow-auto rounded-t-3xl bg-white p-5 shadow-2xl animate-[sheet-in_220ms_ease-out] sm:rounded-3xl sm:p-7 ${compact ? "sm:max-w-md" : "sm:max-w-lg"}`} onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <h2 className="font-serif text-2xl font-bold text-neutral-950 sm:text-3xl">{title}</h2>
...
export function BookingFlow() {
  ...
  return <main className="min-h-screen bg-[#f7faf8] text-neutral-950">
    ...
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-14"><div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-start"><aside className="lg:sticky lg:top-24"><p className="text-xs font-black uppercase tracking-[0.22em] text-teal-950 sm:text-sm">Reserva online</p><h1 className="mt-3 max-w-xl font-serif text-4xl font-bold leading-[1.05] text-neutral-950 sm:mt-4 sm:text-6xl">Agenda tu estilo</h1><p className="mt-4 max-w-md text-base leading-7 text-neutral-600 sm:mt-5 sm:text-lg sm:leading-8">Reserva tu próxima visita en {settings?.business_name ?? "ELYON BARBER"} con una experiencia simple, clara y pensada para mobile.</p></aside>
    ...
    {modal === "success" && service && <Modal title="¡Cita agendada!" onClose={closeSuccess}><div className="text-center"><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-teal-950 text-2xl text-white animate-[check-pop_320ms_ease-out]">✓</div><h3 className="mt-5 font-serif text-3xl font-bold text-neutral-950">Todo listo, {form.name}</h3><p className="mt-3 text-sm leading-6 text-neutral-600">Tu reserva para <strong>{service.name}</strong> quedó registrada para el <strong>{dateLabel}</strong> a las <strong>{formatTime(time)}</strong>.</p>{emailWarning && <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-left text-sm font-bold text-amber-800">{emailWarning}</p>}<Button className="mt-6 w-full" onClick={closeSuccess}>Aceptar</Button></div></Modal>}
  </main>;
}
