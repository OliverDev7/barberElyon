...
export function BookingFlow() {
  ...
  return <main className="min-h-screen bg-[#f7faf8] text-neutral-950">
    ...
    <PublicFooter />
    {pendingService && <Modal title="" compact onClose={() => setPendingService(null)}><div className="flex flex-col gap-4 rounded-2xl border border-teal-100 bg-teal-50/70 p-5 sm:flex-row sm:items-center sm:gap-8 sm:p-6"><div className="min-w-0 flex-1"><h3 className="text-[1.55rem] font-black leading-tight tracking-[-0.025em] text-neutral-950 sm:text-2xl">¿Estás a punto de agendar {pendingService.name}?</h3><p className="mt-3 text-sm font-semibold text-neutral-600 sm:mt-2"><strong>Duración:</strong> {durationLabel(pendingService.duration_minutes)}</p></div><div className="shrink-0 sm:ml-auto"><Button onClick={confirmService}>Confirmar</Button></div></div></Modal>}
