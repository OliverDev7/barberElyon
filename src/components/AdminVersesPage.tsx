...
const textarea = "min-h-32 w-full rounded-lg border border-[#dce9e5] bg-white px-3.5 py-3 text-sm font-semibold text-neutral-900 outline-none transition hover:border-teal-200 focus:border-teal-900 focus:ring-4 focus:ring-teal-50";
...
export function AdminVersesPage() {
  ...
  return <div className="grid gap-5">
    <div><p className="eyebrow">Contenido</p><h2 className="section-title mt-1">Versículos bíblicos</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500">Elige un versículo o introdúcelo manualmente y decide si aparecerá en el Footer, en los correos o en ambos.</p></div>
    ...
    <Panel><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow text-neutral-400">Selector</p><h3 className="mt-1 text-lg font-black">Reina-Valera 1909</h3></div><span className="rounded-full border border-[#dce9e5] bg-[#eef5f3] px-3 py-1 text-[10px] font-black uppercase tracking-wide text-teal-900">Dominio público</span></div><div className="mt-4 grid gap-3 md:grid-cols-3">
    ...
    <Panel><p className="eyebrow text-neutral-400">Dónde mostrarlo</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="flex cursor-pointer items-center gap-3 rounded-lg border border-[#dce9e5] bg-neutral-50 p-4 text-sm font-bold transition hover:bg-[#eef5f3]"><input className="h-5 w-5 accent-teal-900" type="checkbox" checked={verse.show_footer} onChange={(e) => setVerse({ ...verse, show_footer: e.target.checked })} /><span><b className="block">Al pie de página</b><small className="font-medium text-neutral-500">Mostrar el versículo en el Footer.</small></span></label><label className="flex cursor-pointer items-center gap-3 rounded-lg border border-[#dce9e5] bg-neutral-50 p-4 text-sm font-bold transition hover:bg-[#eef5f3]"><input className="h-5 w-5 accent-teal-900" type="checkbox" checked={verse.show_email} onChange={(e) => setVerse({ ...verse, show_email: e.target.checked })} /><span><b className="block">Al pie del correo</b><small className="font-medium text-neutral-500">Añadirlo a nuevos correos de reserva.</small></span></label></div></Panel>
